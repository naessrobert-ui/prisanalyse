...
def _is_room_count_like(series: pd.Series) -> bool:
    v = pd.to_numeric(series, errors="coerce").dropna()
    if v.empty:
        return False
    small_share = ((v >= 1) & (v <= 20)).mean()
    integer_share = (np.abs(v - np.round(v)) < 1e-9).mean()
    return bool((small_share >= 0.7) and (integer_share >= 0.9))
...
def _parse_area_m2(df: pd.DataFrame) -> pd.Series:
    ...
    # Hvis valgt kolonne ser ut som romtall (1..20 heltall), forkast den.
    area_num_preview = pd.to_numeric(
        area.str.replace("m²", "", regex=False).str.replace("m2", "", regex=False).str.replace(" ", "", regex=False).str.replace(",", ".", regex=False),
        errors="coerce",
    )
    if _is_room_count_like(area_num_preview):
        return pd.Series(np.nan, index=df.index, dtype=float)
