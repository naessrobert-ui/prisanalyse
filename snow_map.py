...
def build_top_panel_html(df: pd.DataFrame, region_label: str, n: int = 10) -> str:
    ...
    # JS: vent til vinduet er lastet, finn Leaflet-kartet,
    ...
    js = (
        ...
        "        try{"
        "          var v=window[k];"
        "          if(v && v instanceof L.Map){ mapObj=v; break; }"
        "        }catch(e){}"
        "      }"
        "    }"
        ...
        "          return lat>=b.getSouth() && lat<=b.getNorth() && "
        ...
        "        });"
        "        if(titleEl){"
        "          titleEl.textContent='Topp 10 snøstasjoner – utsnitt';"
        "        }"
        "        attachRowHandlers();"
        "      };"
        "    }"
